...
/**
 * Returns IPs that have a static IP+MAC binding (inet firewall fixed_ips set).
 * Empty when FW_MACFIX is off or nft sets are missing.
 *
 * @return array
 */
...
/**
 * Returns IP => MAC map for static bindings (from fixed_clients set in inet firewall).
 * Empty when FW_MACFIX is off or sets are missing.
 *
 * @return array
 */
function getFixedBindingMap() {
    $map = array();
    foreach (array('inet firewall fixed_clients') as $set) {
        $output = shell_exec('nft list set ' . $set . ' 2>/dev/null');
        if (empty($output)) {
            continue;
        }
